  ...
  useEffect(() => {
    ...
    client.auth.getSession().then(async ({ data }: { data: { session: Session | null } }) => {
      ...
      const { data: rows } = await client
        .from('posts')
        .select('id, title, content, category, created_at, view_count, comment_count, like_count, profiles(nickname, role)')
        .eq('author_id', u.id)
        .eq('is_deleted', false)
        .order('created_at', { ascending: false });

      if (rows) {
        setPosts(rows as unknown as FeedPost[]);
      }
      setLoading(false);
    });
  }, []);
    ...
    <div className="min-h-screen bg-[#F5F6FA] text-[#1A1A1A]">
      ...
      <header className="sticky top-0 z-[200] bg-white border-b border-[#EBEBEB]" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
        <div className="max-w-[600px] mx-auto flex items-center min-h-[54px] px-3 gap-2">
          ...
          <span className="text-[15px] font-bold">내가 쓴 글</span>
        </div>
      </header>
      ...
      <div className="max-w-[600px] mx-auto px-4 pt-4 pb-tabbar">
        ...
        {!loading && user && posts.length === 0 && (
          <div className="flex flex-col items-center py-16 text-center">
            <p className="text-[15px] text-gray-400">내가 쓴 글이 없어요</p>
            <p className="text-[13px] text-gray-300 mt-1">커뮤니티에 첫 글을 작성해 보세요</p>
          </div>
        )}
        ...
        {!loading && posts.length > 0 && (
          <div className="space-y-2.5">
            {posts.map(post => (
              ...
              >
                {/* 카테고리 필 */}
                <span className="inline-block text-[11px] text-[#1D4ED8] font-semibold bg-[#EFF6FF] px-2 py-0.5 rounded-full mb-2">
